...
# utilizando o método obter_contato() e apresentando os resultados
print('contato usuario1:', usuario1.obter_contato())
print('contato usuario2:', usuario2.obter_contato())
print('contato usuario3:', usuario3.obter_contato())
\end{minted}
\begin{minted}{text}
contato usuario1: fulano@ifsc.edu.br
contato usuario2: (48)99999-8888
...
\inlcode{obter_contato()} (\textbf{linha 9}).
...
Na \textbf{linha 23}, dentro do comando \inlcode{print()}, temos a chamada  \inlcode{usuario1.obter_contato()}, que executa o
método \inlcode{obter_contato()} definido na \textbf{linha 9}.

Um detalhe importante é que, na definição de \inlcode{contato(}\magic{self}\inlcode{)},
declaramos um parâmetro
...
Ou seja, a chamada \inlcode{usuario1.obter_contato()} equivale
internamente a algo como \inlcode{obter_contato(}\magic{self}\inlcode{=usuario1)}, onde \magic{self} recebe
...
Embora Python não imponha um mecanismo formal de privacidade, convencionou-se utilizar um
underscore (\inlcode{_}) antes do nome do atributo para indicar que ele
...
\pagebreak
Em outro exemplo, poderíamos criar uma segunda classe denominada \inlcode{Cadastro}, que agrupa diversos
usuários e ofereçe funcionalidades sobre essa coleção:
\begin{minted}[linenos]{custompython}
class Usuario:
...
class Cadastro:
    ...
    def procurar_usuario(self, nome: str) -> Usuario | None:
        for usuario in self.usuarios:
            if usuario.nome == nome:
                return usuario
        return None

    def imprimir_contato(self, nome: str):
        selecionado = cadastro.procurar_usuario(nome)
        if isinstance(selecionado, Usuario):
            print(f"O contato de {selecionado.nome} é: {selecionado.obter_contato()}")
        else:
            print(f"Usuário '{nome}' não encontrado.")

cadastro = Cadastro()
cadastro.adicionar_usuario(Usuario('Fulano', 'fulano@ifsc.edu.br', '(48)99999-9999'))
cadastro.adicionar_usuario(Usuario('Beltrano', telefone='(48)99999-8888'))
cadastro.adicionar_usuario(Usuario('Cicrano'))

cadastro.imprimir_contato("Beltrano")
\end{minted}
\begin{minted}{text}
O contato de Beltrano é: (48)99999-8888
\end{minted}
